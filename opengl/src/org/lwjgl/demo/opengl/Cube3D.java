...
class CubeModel extends OGLModel3D {
    ...
    public void render() {
        ...
        // front
        M.translation(0, 0, 1).rotateY(Math.PI);
        drawSide(m_side.setRGBA(1, 0, 0, 1));

        // right
        M.rotationY(Math.PI/2).translate(0, 0, 1).rotateY(Math.PI);
        drawSide(m_side.setRGBA(1, 1, 0, 1));

        // back
        M.rotationY(Math.PI).translate(0, 0, 1).rotateY(Math.PI);
        ...
        // bottom
        M.rotationX(Math.PI/2).translate(0, 0, 1).rotateX(Math.PI);
